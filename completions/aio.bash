...
_aio( {
  ...
  case "${prev}" in
  -p | --pattern)
    COMPREPLY=($(compgen -W "$(_aio_get_list --listpatterns)" -- "${cur}"))
    return 0
    ;;
  -C | --context)
    COMPREPLY=($(compgen -W "$(_aio_get_list --listcontexts)" -- "${cur}"))
    return 0
    ;;
  --session)
    COMPREPLY=($(compgen -W "$(_aio_get_list --listsessions)" -- "${cur}"))
    return 0
    ;;
  -m | --model)
    COMPREPLY=($(compgen -W "$(_aio_get_list --listmodels)" -- "${cur}"))
    return 0
    ;;
  -V | --vendor)
    COMPREPLY=($(compgen -W "$(_aio_get_list --listvendors)" -- "${cur}"))
    return 0
    ;;
  -w | --wipecontext)
    COMPREPLY=($(compgen -W "$(_aio_get_list --listcontexts)" -- "${cur}"))
  ...
  --thinking)
    ...
    ;;
  --rmextension)
    COMPREPLY=($(compgen -W "$(_aio_get_list --listextensions)" -- "${cur}"))
    return 0
    ;;
  --strategy)
    COMPREPLY=($(compgen -W "$(_aio_get_list --liststrategies)" -- "${cur}"))
    return 0
    ;;
  --voice)
    COMPREPLY=($(compgen -W "$(_aio_get_list --list-gemini-voices)" -- "${cur}"))
    return 0
    ;;
  --transcribe-model)
    COMPREPLY=($(compgen -W "$(_aio_get_list --list-transcription-models)" -- "${cur}"))
    return 0
    ;;
  --debug)
