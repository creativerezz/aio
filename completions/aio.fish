# Fish shell completion for aio CLI
#
# Installation:
# Copy this file to ~/.config/fish/completions/aio.fish
# or run:
# mkdir -p ~/.config/fish/completions
# cp completions/aio.fish ~/.config/fish/completions/

# Helper functions for dynamic completions
function __aio_get_patterns
        set cmd (commandline -opc)[1]
        $cmd --listpatterns --shell-complete-list 2>/dev/null
end

function __aio_get_models
        set cmd (commandline -opc)[1]
        $cmd --listmodels --shell-complete-list 2>/dev/null
end

function __aio_get_vendors
        set cmd (commandline -opc)[1]
        $cmd --listvendors --shell-complete-list 2>/dev/null
end

function __aio_get_contexts
        set cmd (commandline -opc)[1]
        $cmd --listcontexts --shell-complete-list 2>/dev/null
end

function __aio_get_sessions
        set cmd (commandline -opc)[1]
        $cmd --listsessions --shell-complete-list 2>/dev/null
end

function __aio_get_strategies
        set cmd (commandline -opc)[1]
        $cmd --liststrategies --shell-complete-list 2>/dev/null
end

function __aio_get_extensions
        set cmd (commandline -opc)[1]
        $cmd --listextensions --shell-complete-list 2>/dev/null
end

function __aio_get_gemini_voices
        set cmd (commandline -opc)[1]
        $cmd --list-gemini-voices --shell-complete-list 2>/dev/null
end

function __aio_get_transcription_models
        set cmd (commandline -opc)[1]
        $cmd --list-transcription-models --shell-complete-list 2>/dev/null
end

# Main completion function
function __aio_register_completions
        set cmd $argv[1]
        complete -c $cmd -f

        # Flag completions with arguments
        complete -c $cmd -s p -l pattern -d "Choose a pattern from the available patterns" -a "(__aio_get_patterns)"
        complete -c $cmd -s v -l variable -d "Values for pattern variables, e.g. -v=#role:expert -v=#points:30"
        complete -c $cmd -s C -l context -d "Choose a context from the available contexts" -a "(__aio_get_contexts)"
        complete -c $cmd -l session -d "Choose a session from the available sessions" -a "(__aio_get_sessions)"
        ...
        complete -c $cmd -s m -l model -d "Choose model" -a "(__aio_get_models)"
        complete -c $cmd -s V -l vendor -d "Specify vendor for chosen model (e.g., -V \"LM Studio\" -m openai/gpt-oss-20b)" -a "(__aio_get_vendors)"
        ...
        complete -c $cmd -s w -l wipecontext -d "Wipe context" -a "(__aio_get_contexts)"
        complete -c $cmd -s W -l wipesession -d "Wipe session" -a "(__aio_get_sessions)"
        complete -c $cmd -l printcontext -d "Print context" -a "(__aio_get_contexts)"
        complete -c $cmd -l printsession -d "Print session" -a "(__aio_get_sessions)"
        ...
        complete -c $cmd -l rmextension -d "Remove a registered extension by name" -a "(__aio_get_extensions)"
        complete -c $cmd -l strategy -d "Choose a strategy from the available strategies" -a "(__aio_get_strategies)"
        ...
        complete -c $cmd -l voice -d "TTS voice name for supported models (e.g., Kore, Charon, Puck)" -a "(__aio_get_gemini_voices)"
        complete -c $cmd -l transcribe-file -d "Audio or video file to transcribe" -r -a "*.mp3 *.mp4 *.mpeg *.mpga *.m4a *.wav *.webm"
        complete -c $cmd -l transcribe-model -d "Model to use for transcription (separate from chat model)" -a "(__aio_get_transcription_models)"
        ...
        complete -c $cmd -l serve -d "Serve the Aio Rest API"
        complete -c $cmd -l serveOllama -d "Serve the Aio Rest API with ollama endpoints"
