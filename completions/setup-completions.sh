...
# Aio Shell Completions Setup Script
# This script automatically installs shell completions for the aio CLI
# based on your current shell and the installed aio command name.
...
# Can be overridden via environment variable AIO_COMPLETIONS_BASE_URL
AIO_COMPLETIONS_BASE_URL="${AIO_COMPLETIONS_BASE_URL:-https://raw.githubusercontent.com/creativerezz/aio/refs/heads/main/completions}"
TEMP_DIR=""
...
obtain_completion_files() {
    ...
    if [ ! -f "$obf_script_dir//_aio" ] || [ ! -f "$obf_script_dir/aio.bash" ] || [ ! -f "$obf_script_dir/aio.fish" ]; then
    ...
    printf "%s\n" "[INFO] Source: $AIO_COMPLETIONS_BASE_URL" 1>&2
    ...
    TEMP_DIR="$(mktemp -d 2>/dev/null || mktemp -d -t aio-completions)"
    if [ ! -d "$TEMP_DIR" ]; then
    print_error "Failed to create temporary directory for downloads."
        return 1
    fi

    if ! download_file "$AIO_COMPLETIONS_BASE_URL//_aio" "$TEMP_DIR//_aio"; then
        print_error "Failed to download /_aio"
        return 1
    fi
    if [ ! -s "$TEMP_DIR//_aio" ] || head -n1 "$TEMP_DIR//_aio" | grep -qi "^<!DOCTYPE\|^<html"; then
        print_error "Downloaded /_aio appears invalid (empty or HTML). Check AIO_COMPLETIONS_BASE_URL."
        return 1
    fi
    if ! download_file "$AIO_COMPLETIONS_BASE_URL/aio.bash" "$TEMP_DIR/aio.bash"; then
        print_error "Failed to download aio.bash"
        return 1
    fi
    if [ ! -s "$TEMP_DIR/aio.bash" ] || head -n1 "$TEMP_DIR/aio.bash" | grep -qi "^<!DOCTYPE\|^<html"; then
        print_error "Downloaded aio.bash appears invalid (empty or HTML). Check AIO_COMPLETIONS_BASE_URL."
        return 1
    fi
    if ! download_file "$AIO_COMPLETIONS_BASE_URL/aio.fish" "$TEMP_DIR/aio.fish"; then
        print_error "Failed to download aio.fish"
        return 1
    fi
    if [ ! -s "$TEMP_DIR/aio.fish" ] || head -n1 "$TEMP_DIR/aio.fish" | grep -qi "^<!DOCTYPE\|^<html"; then
        print_error "Downloaded aio.fish appears invalid (empty or HTML). Check AIO_COMPLETIONS_BASE_URL."
        return 1
    fi
...
setup_zsh_completions() {
    aio_cmd="$1"
    script_dir="$2"
    completion_file="_${aio_cmd}"

    print_info "Setting up Zsh completions for '$aio_cmd'..."
    ...
    for dir in $zsh_dirs; do
        ...
        if ensure_dir "$dir"; then
            if install_file "$script_dir//_aio" "$dir/$completion_file"; then
                if [ "$DRY_RUN" = true ]; then
...
setup_bash_completions() {
    aio_cmd="$1"
    script_dir="$2"
    completion_file="${aio_cmd}.bash"

    print_info "Setting up Bash completions for '$aio_cmd'..."
    ...
    for dir in $bash_dirs; do
        if ensure_dir "$dir"; then
            if install_file "$script_dir/aio.bash" "$dir/$completion_file"; then
                if [ "$DRY_RUN" = true ]; then
...
setup_fish_completions() {
    aio_cmd="$1"
    script_dir="$2"
    completion_file="${aio_cmd}.fish"

    print_info "Setting up Fish completions for '$aio_cmd'..."
    ...
    if [ "$DRY_RUN" = true ]; then
        ...
        print_dry_run "Would run: cp \"$script_dir/aio.fish\" \"$fish_dir/$completion_file\""
        ...
        if cp "$script_dir/aio.fish" "$fish_dir/$completion_file"; then
...
setup_other_shell_completions() {
    ...
    print_info "  Bash-compatible: source $script_dir/aio.bash"
    print_info "  Zsh-compatible: source $script_dir//_aio"
}
...
DESCRIPTION:
    ...
    based on your current shell and the installed aio command name.
        ...
        will download them from GitHub:
            ...
            $AIO_COMPLETIONS_BASE_URL

        You can override the download source by setting
        AIO_COMPLETIONS_BASE_URL to your preferred location.

    Supports: zsh, bash, fish

    The script will:
    1. Detect whether 'aio' is installed
...
EXAMPLES:
        ...
        AIO_COMPLETIONS_BASE_URL="https://raw.githubusercontent.com/<owner>/<repo>/main/completions" \\
...
main() {
    ...
    print_info "Aio Shell Completions Setup"
    print_info "==============================="
    ...
    case "$shell_name" in
        zsh)
            setup_zsh_completions "$aio_cmd" "$script_dir"
            ;;
        bash)
            setup_bash_completions "$aio_cmd" "$script_dir"
            ;;
        fish)
            setup_fish_completions "$aio_cmd" "$script_dir"
            ;;
        *)
            setup_other_shell_completions "$aio_cmd" "$shell_name" "$script_dir"
            ;;
    esac
    ...
    if [ "$DRY_RUN" = true ]; then
        ...
        print_info "You can now use tab completion with the '$aio_cmd' command."
    fi
}
