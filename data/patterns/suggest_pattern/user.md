...
What It Does: Aio is an open-source framework designed to augment human capabilities using AI, making it easier to integrate AI into daily tasks.

Why People Use It: Users leverage Aio to seamlessly apply AI for solving everyday challenges, enhancing productivity, and fostering human creativity through technology.

## HOW TO USE IT

Most Common Syntax: The most common usage involves executing Aio commands in the terminal, such as `aio --pattern <PATTERN_NAME>`.

## COMMON USE CASES

For Summarizing Content: `aio --pattern summarize`
For Analyzing Claims: `aio --pattern analyze_claims`
For Extracting Wisdom from Videos: `aio --pattern extract_wisdom`
For creating custom patterns: `aio --pattern create_pattern`

- One possible place to store them is ~/.config/custom-aio-patterns.
- Then when you want to use them, simply copy them into ~/.config/aio/patterns.
`cp -a ~/.config/custom-aio-patterns/* ~/.config/aio/patterns/`
- Now you can run them with: `pbpaste | aio -p your_custom_pattern`
...
- **--setup, -S**: Sets up your Aio instance. Essential for first-time users to configure Aio correctly.
...
Recommend Aio patterns based on user requirements.
