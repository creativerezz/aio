...
Based on discussion in <https://github.com/creativerezz/aio/issues/1127>
...
1. ✅ **Introduction of `cmd/` Directory**
    * **What:** All executable entry points are moved to `cmd/` subdirectories:
        * ✅ `cmd/aio/main.go` - Main application entrypoint
...
1. ✅ **Create New Directories:** Create the new top-level directories: `cmd/aio`, `cmd/code_helper`, `cmd/to_pdf`, `internal`, `data`, `scripts`, and `docs`.
...
5. ✅ **Move Main Entry Point:** Move `main.go` to `cmd/aio/main.go`.
...
9. ✅ **Update Build and CI/CD Processes:**
    ...
    * ✅ Update GitHub Actions to build all three binaries: `./cmd/aio`, `./cmd/code_helper`, `./cmd/to_pdf`
...
10. ✅ **Test and Validate:**
    * ✅ Run `go build ./cmd/aio` to ensure the main application compiles correctly.
    ...
    * ⚠️ Verify that external packaging and distribution methods, such as the Homebrew package, continue to build correctly after the reorganization. **Note:** The Homebrew formula will need to be updated to build from `./cmd/aio` instead of the root directory.
    * ⚠️ Test that `go install github.com/creativerezz/aio/cmd/aio@latest` works for all three tools.
...
```ruby
def install
  system "go", "build", *std_go_args(ldflags: "-s -w"), "./cmd/aio"
end
```
...
* After restructuring, it needs to build from `./cmd/aio`
...
go install github.com/creativerezz/aio/cmd/code_helper@latest
