# Shell Completions for Aio

> Aio is forked from [danielmiessler/fabric](https://github.com/danielmiessler/fabric). Credit to the original Fabric project and its contributors.

Aio comes with shell completion support for Zsh, Bash, and Fish shells. These completions provide intelligent tab-completion for commands, flags, patterns, models, contexts, and more.
...
- Detect whether you have `aio` installed
...
1. Copy the completion file to a directory in your `$fpath`:
   ...
   ```bash
   sudo cp completions/_aio /usr/local/share/zsh/site-functions/
   ```
...
1. Copy the completion file to a standard completion directory:
   ...
   sudo cp completions/aio.bash /etc/bash_completion.d/

   # Or user-specific installation
   mkdir -p ~/.local/share/bash-completion/completions/
   cp completions/aio.bash ~/.local/share/bash-completion/completions/aio
   ```
...
2. Fish will automatically load the completions (no restart needed).
...
- **Zsh**: Add to `~/.zshrc`: `source /path/to/aio/completions/_aio`
- **Bash**: Add to `~/.bashrc`: `source /path/to/aio/completions/aio.bash`
...
The completion system dynamically queries the `aio` command for current patterns, models, and other resources, so your completions will always be up-to-date with your installation.
