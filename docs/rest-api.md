...
| `patternName` | No | `""` | Pattern to apply (from `~/.config/aio/patterns/`) |
| `contextName` | No | `""` | Context to prepend (from `~/.config/aio/contexts/`) |
| `strategyName` | No | `""` | Strategy to use (from `~/.config/aio/strategies/`) |
...
Updates `~/.config/aio/.env` with new values.
...
docker run --rm -it \
  -v $HOME/.fabric-config:/root/.config/aio \
