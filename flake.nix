    ...
    in
    {
      ...
      packages = forAllSystems (
        ...
        let
          pkgs = nixpkgs.legacyPackages.${system};
          goVersion = getGoVersion system;
          aioSlim = pkgs.callPackage ./nix/pkgs/aio {
            go = goVersion;
            inherit self;
            inherit (gomod2nix.legacyPackages.${system}) buildGoApplication;
          };
          aio = pkgs.symlinkJoin {
            name = "aio-${aioSlim.version}";
            inherit (aioSlim) version;
            paths = [
              aioSlim
              pkgs.yt-dlp
            ];
            ...
            '';
            meta = aioSlim.meta // {
              description = "${aioSlim.meta.description} (includes yt-dlp)";
              mainProgram = "aio";
            };
          };
        in
