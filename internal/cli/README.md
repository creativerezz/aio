...
Aio supports YAML configuration files for commonly used options. This allows users to persist settings and share configurations across multiple runs.
...
```bash
aio --config ~/.config/aio/config.yaml "Tell me about APIs"
```
...
aio --config ~/.config/aio/config.yaml --temperature 0.9 "Query"
```
