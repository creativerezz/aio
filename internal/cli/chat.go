...
import (
	...
	"github.com/creativerezz/aio/internal/tools/notifications"
)
...
func handleChatProcessing(currentFlags *Flags, registry *core.PluginRegistry, messageTools string) (err error) {
	...
	if currentFlags.Pattern != "" && currentFlags.Model == "" {
		envVar := "AIO_MODEL_" + strings.ToUpper(strings.ReplaceAll(currentFlags.Pattern, "-", "_"))
	...
	if !currentFlags.Stream || currentFlags.SuppressThink {
		...
		if isTTSModel && isAudioOutput && strings.HasPrefix(result, "AIO_AUDIO_DATA:") {
	...
	if currentFlags.Output != "" {
		if currentFlags.OutputSession {
			...
			if isTTSModel && isAudioOutput {
				...
				if strings.HasPrefix(result, "AIO_AUDIO_DATA:") {
					// Extract the binary audio data
					audioData := result[len("AIO_AUDIO_DATA:"):]
