...
func Init() (ret *Flags, err error) {
	...
	// Check to see if a ~/.config/aio/config.yaml config file exists (only when user didn't specify a config)
	if ret.Config == "" {
		// Default to ~/.config/aio/config.yaml if no config specified
