...
# Aio Extensions: Complete Guide

## Important: Extensions Only Work in Patterns

**Extensions are ONLY processed when used within pattern files, not via direct piping to aio.**

```bash
# ❌ This DOES NOT WORK - extensions are not processed in stdin
echo "{{ext:word-generator:generate:3}}" | aio

# ✅ This WORKS - extensions are processed within patterns
aio -p my-pattern-with-extensions.md
```

When you pipe directly to aio without a pattern, the input goes straight to the LLM without template processing. Extensions are only evaluated during pattern template processing via `ApplyTemplate()`.

## Understanding Extension Architecture

### Registry Structure

The extension registry is stored at `~/.config/aio/extensions/extensions.yaml` and tracks registered extensions:
...
```bash
# Create directories
mkdir -p ~/.config/aio/extensions/{bin,configs}

# Install script
cp word-generator.py ~/.config/aio/extensions/bin/
chmod +x ~/.config/aio/extensions/bin/word-generator.py
```

### 2. Configure

Create `~/.config/aio/extensions/configs/word-generator.yaml`:

```yaml
name: word-generator
executable: "~/.config/aio/extensions/bin/word-generator.py"
type: executable
timeout: "5s"
...
```bash
# Register
aio --addextension ~/.config/aio/extensions/configs/word-generator.yaml

# Extensions must be used within patterns (see "Extensions in patterns" section below)
# Direct piping to aio will NOT process extension syntax
```
...
Create `~/.config/aio/extensions/configs/memory-query.yaml`:
...
```bash
# Register
aio --addextension ~/.config/aio/extensions/configs/memory-query.yaml
...
```bash
aio --rmextension <extension-name>
```
...
**IMPORTANT**: Extensions are ONLY processed when used within pattern files, not via direct piping to aio.
...
```bash
aio -p ./internal/plugins/template/Examples/test_pattern.md
```
...
```bash
echo "What is Artificial Intelligence" | ../aio-fix -p ai_summarize
