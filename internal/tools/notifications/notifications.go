...
func (o *OSAScriptProvider) Send(title, message string) error {
	...
	script := `display notification (system attribute "AIO_MESSAGE") with title (system attribute "AIO_TITLE") sound name "Glass"`
	cmd := exec.Command("osascript", "-e", script)

	// Set environment variables for the AppleScript to read safely
	cmd.Env = append(os.Environ(), "AIO_TITLE="+title, "AIO_MESSAGE="+message)
	return cmd.Run()
}
...
func (p *PowerShellProvider) Send(title, message string) error {
	...
	script := `Add-Type -AssemblyName System.Windows.Forms; [System.Windows.Forms.MessageBox]::Show($env:AIO_MESSAGE, $env:AIO_TITLE)`
