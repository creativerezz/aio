...
# File Handler
log_dir = os.path.expanduser("~/.config/aio/logs")
...
# Global variables
pattern_dir = os.path.expanduser("~/.config/aio/patterns")
MAX_RETRIES = 3
...
def parse_models_output(output: str) -> Dict[str, List[str]]:
    """Parse the output of aio --listmodels command."""
...
def fetch_models_once() -> Dict[str, List[str]]:
    ...
    success, stdout, stderr = safe_run_command(["aio", "--listmodels"])
    if not success:
...
def get_configured_providers() -> Dict[str, List[str]]:
    """Get list of configured providers using aio --listmodels."""
...
def load_configuration() -> bool:
    ...
    try:
        env_path = os.path.expanduser("~/.config/aio/.env")
        ...
        if not providers:
            ...
            st.error("No providers configured. Please run 'aio --setup' first.")
            return False
...
def load_models_and_providers() -> None:
    """Load models and providers from aio configuration."""
    try:
...
def create_pattern(
    ...
    try:
        ...
        # If content is provided, use aio create_pattern to structure it
        if content:
            logger.info(
            ...
            try:
                ...
                # Execute aio create_pattern with input content
                cmd = ["aio", "--pattern", "create_pattern"]
...
def get_outputs_dir() -> str:
    ...
    outputs_dir = os.path.expanduser("~/.config/aio/outputs")
    ...
    try:
        ...
        st.markdown(
            ...
            <div class="aio-header">
                <h1 class="aio-title">Pattern Studio</h1>
            </div>
            <div class="assistant-container">
                <div class="assistant-avatar" onclick="window.open('https://github.com/creativerezz/aio', '_blank')"></div>
            </div>
        ...
        with st.sidebar:
            ...
            st.markdown(
                ...
                <div style='text-align: center; margin-bottom: 1rem;'>
                    <a href="https://github.com/creativerezz/aio" target="_blank">
                        <img src="https://img.shields.io/github/stars/creativerezz/aio?style=social" alt="GitHub Repo">
                    </a>
                </div>
                """,
                unsafe_allow_html=True,
