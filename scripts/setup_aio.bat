...
if not "!YOUTUBE_API_KEY!"=="" (
    echo YOUTUBE_API_KEY=!YOUTUBE_API_KEY!>> %USERPROFILE%\.config\aio\.env
)

:: Prompt user for OpenAI API Key
set /p OPENAI_API_KEY=Enter your OpenAI API Key (press Enter to skip): 
if not "!OPENAI_API_KEY!"=="" (
    echo OPENAI_API_KEY=!OPENAI_API_KEY!>> %USERPROFILE%\.config\aio\.env
)

:: Run Aio
:run_aio
cls
echo Aio is now installed and ready to use.
echo.
echo Available options:
echo 1. Run Aio with custom options
...
if "%CHOICE%"=="1" (
    ...
    set "AIO_CMD=aio"
    if not "!PATTERN!"=="" set "AIO_CMD=!AIO_CMD! --pattern !PATTERN!"
    if not "!CONTEXT!"=="" set "AIO_CMD=!AIO_CMD! --context !CONTEXT!"
    if not "!SESSION!"=="" set "AIO_CMD=!AIO_CMD! --session !SESSION!"
    if not "!MODEL!"=="" set "AIO_CMD=!AIO_CMD! --model !MODEL!"
    if not "!TEMPERATURE!"=="" set "AIO_CMD=!AIO_CMD! --temperature !TEMPERATURE!"
    if /i "!STREAM!"=="Y" set "AIO_CMD=!AIO_CMD! --stream"

    echo Running Aio with command: !AIO_CMD!
    !AIO_CMD!
    pause
    goto run_aio
) else if "%CHOICE%"=="2" (
